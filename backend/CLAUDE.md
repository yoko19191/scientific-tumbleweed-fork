...
Scientific Tumbleweed is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, persistent memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.
...
│   │   │   └── routers/       # FastAPI route modules (models, mcp, memory, skills, uploads, threads, artifacts, agents, apps, suggestions, channels)
...
- **Workspace apps**: `deerflow.apps` is the harness-level registry for focused app modules surfaced by the web UI. Gateway exposes registered definitions through `app.gateway.routers.apps` at `/api/apps`; do not hard-code placeholder app cards in the frontend. The built-in `research-data-search` app is registered from `deerflow.apps.research_data_search`, while its authenticated data proxy, DTO normalization, and upstream credential handling live in Gateway app-layer modules.
- **Runs and checkpoints**: `deerflow.runtime.runs.checkpoints` owns root-checkpoint config construction, raw channel-value extraction, and serialized final-state reads for wait endpoints. `RunRecord.from_store_row()` owns persistent `RunStore` row hydration. `RunManager` keeps live runtime records indexed by thread and inflight status; do not replace hot-path lookups with full `_runs` scans. `run_agent()` schedules delayed runtime cleanup after stream cleanup, while historical run reads continue through `RunStore`. `deerflow.runtime.format_sse_frame()` owns the LangGraph-compatible SSE frame shape; Gateway `format_sse()` is only a wrapper.
...
**Lead Agents** (`packages/harness/deerflow/agents/lead_agent/`):
- Entry points: `make_chat_lead_agent(config: RunnableConfig)` and `make_computer_lead_agent(config: RunnableConfig)` registered in `langgraph.json` as `chat_lead_agent` and `computer_lead_agent`
- Module shape: `base.py` owns shared `create_agent` assembly, middleware/model/tool resolution, and bootstrap handling; `config.py` owns typed `LeadProfile` defaults; `chat.py` and `computer.py` are thin variant entrypoints; `agent.py` is only a backward-compatible facade.
- Chat uses the local filesystem sandbox provider, file/web/academic tools, plan mode, and subagents without shell execution; Computer keeps the full workspace/sandbox tool surface
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` and the unified Jinja2 `build_prompt()` path; lead-agent prompt helpers are split across `prompt.py`, `skills_cache.py`, `skills_prompt.py`, and `dynamic_sections.py`
- Suggestion and title generation are harness agent modules: `agents/suggestion_agent` owns follow-up prompt rendering/parsing/model calls, and `agents/title_agent` owns title prompt rendering/parsing/fallback/model calls. Gateway routers and `TitleMiddleware` remain thin boundary layers.
...
Lead-agent and SDK middlewares are ordered by `packages/harness/deerflow/agents/middleware_builder.py`. `make_chat_lead_agent()`, `make_computer_lead_agent()`, and `create_deerflow_agent()` may resolve which concrete slots are enabled, but final relative order must go through `build_ordered_middleware_chain()`. `extra_middleware` insertion belongs in `insert_extra_middlewares()` so `@Next`, `@Prev`, circular-anchor detection, and the clarification tail invariant stay shared.
...
| **Threads** (`/api/threads/{id}`) | `DELETE /` - remove Scientific Tumbleweed-managed local thread data after LangGraph thread deletion; unexpected failures are logged server-side and return a generic 500 detail |
...
Bridges external messaging platforms (Feishu, Slack, Telegram) to the Scientific Tumbleweed agent via the LangGraph Server.
...
**Data Structure** (stored in Postgres table `user_memory`, one row per user; `user_id='__global__'` for the legacy unscoped memory). Prior to the migration this lived in `backend/.deer-flow/memory.json` — see `backend/docs/MIGRATION.md`:
...
- Agent created lazily via `create_agent()` + `_build_middlewares()`, same as the LangGraph factories; both entry points use the canonical middleware builder
