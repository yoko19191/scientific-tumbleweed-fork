...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Scientific Tumbleweed.
...
Scientific Tumbleweed uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Scientific Tumbleweed prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Scientific Tumbleweed's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Scientific Tumbleweed-style prompts**
...
- Custom prompts emphasize the same principles as Scientific Tumbleweed's main system prompt (clarity, action-oriented, critical rules)
