...
- **Workspace apps**: `deerflow.apps` is the harness-level registry for focused app modules surfaced by the web UI. Gateway exposes registered definitions through `app.gateway.routers.apps` at `/api/apps`; do not hard-code placeholder app cards in the frontend.
