...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `scientific-tumbleweed` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `scientific-tumbleweed` | Kubernetes namespace for sandbox resources |
...
| `USERDATA_PVC_NAME` | empty (use hostPath) | PVC name for user-data volume; when set, uses PVC with `subPath: deer-flow/users/{user_id}/threads/{thread_id}` |
...
Older provisioner versions mounted PVC user-data from `threads/{thread_id}/user-data`. The user-scoped layout mounts from `deer-flow/users/{user_id}/threads/{thread_id}`.
...
This moves legacy `threads/{thread_id}/user-data` data under `users/<target-user-id>/threads/{thread_id}`, which matches the new provisioner PVC subPath when the gateway base directory is mounted at `deer-flow/` on the PVC. Use `default` as the target user only when the legacy data should remain in the default no-auth user namespace. Run the migration while no gateway or sandbox Pods are writing to those paths.
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `scientific-tumbleweed` namespace
     - Create/read/delete Services in the `scientific-tumbleweed` namespace
     - Read Namespaces (to create `scientific-tumbleweed` if missing)
...
docker compose -p scientific-tumbleweed-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec scientific-tumbleweed-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001","user_id":"user-001"}'

# Check sandbox status
docker exec scientific-tumbleweed-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec scientific-tumbleweed-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n scientific-tumbleweed -l sandbox-id=test-001

# Delete sandbox
docker exec scientific-tumbleweed-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec scientific-tumbleweed-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec scientific-tumbleweed-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec scientific-tumbleweed-provisioner ls -ld /root/.kube/config
  ```
...
3. **Network Isolation**: Sandbox Pods run in the `scientific-tumbleweed` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
