...
## 当前项目用户资源隔离总览 2026-04-17

  存储层隔离

  所有用户数据存储在 {base_dir}/users/{user_id}/ 下，物理路径完全隔离：
  ...
  │       ├── memory.json                  # 用户长期记忆
  │       ├── USER.md                      # 用户画像/偏好
  │       ├── extensions_config.json       # MCP + Skill 启用状态
  │       ├── agents/                      # 自定义 Agent 配置
  │       │   └── {agent-name}/
  │       │       ├── config.yaml
  │       │       └── SOUL.md
  │       ├── skills/custom/               # 自定义 Skill
  │       │   └── {skill-name}/SKILL.md
  │       └── threads/                     # Thread 文件资源
  │           └── {thread_id}/
  │               ├── uploads/
  ...
  │         资源         │                    隔离方式                    │            说明             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Thread 列表          │ Store namespace ("threads", "user:{user_id}")  │ 用户只能看到自己的 thread   │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Thread               │ require_thread_owner() 校验                    │ 跨用户访问返回 404          │
  │ CRUD/State/History   │                                                │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Runs 创建/流/取消    │ require_thread_owner() + metadata 注入         │ run 绑定到 thread owner     │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Memory (memory.json) │ get_current_user_id 强制认证 + 文件路径隔离    │ 无全局 fallback             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Uploads/Artifacts    │ owner check +                                  │ 文件物理隔离                │
  │                      │ users/{user_id}/threads/{tid}/uploads/         │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Custom Skill CRUD    │ get_current_user_id +                          │ 创建/编辑/删除/回滚         │
  │                      │ users/{user_id}/skills/custom/                 │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Skill 启用/禁用      │ users/{user_id}/extensions_config.json         │ 用户 A 禁用不影响用户 B     │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Skill Prompt 注入    │ load_skills(user_id=user_id)                   │ Agent 只看到当前用户启用的  │
  │                      │                                                │ skill                       │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Custom Agent CRUD    │ get_current_user_id + users/{user_id}/agents/  │ 创建/编辑/删除              │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Agent Soul/Config    │ load_agent_config(name, user_id=user_id)       │ Agent                       │
  │                      │                                                │ 运行时加载用户自己的配置    │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ User Profile         │ users/{user_id}/USER.md                        │ 注入到 agent prompt         │
  │ (USER.md)            │                                                │ 的用户画像                  │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ MCP Server 启用/禁用 │ 全局定义只读 +                                 │ 用户只能切换开关            │
  │                      │ users/{user_id}/extensions_config.json 覆盖    │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Assistants 列表      │ get_current_user_id + 加载用户自定义 agent     │ 列表包含用户自己的 agent    │
  │                      │ 作为 assistant                                 │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Suggestions          │ require_thread_owner()                         │ 只能对自己的 thread         │
  │                      │                                                │ 生成建议                    │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Channels 管理        │ get_current_user_id                            │ 需要认证才能查看/重启       │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Sandbox (AIO)        │ user_id:thread_id 作为 cache key +             │ 容器内文件物理隔离          │
  │                      │ user-prefixed mount 路径                       │                             │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ Thread Title 同步    │ user_threads_namespace(user_id)                │ Store 记录写入用户          │
  │                      │                                                │ namespace                   │
  ├──────────────────────┼────────────────────────────────────────────────┼─────────────────────────────┤
  │ 前端缓存             │ queryKey 含 userId + sessionStorage key 含     │ 切换账号时缓存不混          │
  │                      │ u:{userId}: 前缀                               │                             │
  └──────────────────────┴────────────────────────────────────────────────┴─────────────────────────────┘
  ...
  │         资源         │                                     原因                                      │
  ├──────────────────────┼───────────────────────────────────────────────────────────────────────────────┤
  │ Models 列表          │ 模型是全局共享资源，所有用户看到相同的可用模型                                │
  ├──────────────────────┼───────────────────────────────────────────────────────────────────────────────┤
  │ Public Skills        │ 公共 skill 是平台提供的，所有用户共享定义，只有启用状态按用户隔离             │
  ├──────────────────────┼───────────────────────────────────────────────────────────────────────────────┤
  │ MCP Server 定义      │ 全局 extensions_config.json 中的 MCP server 定义是共享的，用户只能覆盖        │
  │                      │ enabled 状态                                                                  │
  ├──────────────────────┼───────────────────────────────────────────────────────────────────────────────┤
  │ LangGraph            │ thread state 存在共享 checkpointer 中，通过 thread ownership                  │
  │ Checkpointer         │ 做访问控制而非物理隔离                                                        │
  ├──────────────────────┼───────────────────────────────────────────────────────────────────────────────┤
  │ PG / Minio           │ 未引入，当前用文件系统 + LangGraph InMemoryStore 实现隔离                     │
  └──────────────────────┴───────────────────────────────────────────────────────────────────────────────┘

  隔离的核心机制

  1. 认证层：get_current_user_id（强制 401）用于所有用户资源路由，get_optional_user_id 仅保留在 runs/threads
  等需要兼容匿名模式的路径
  2. Thread 所有权：require_thread_owner() 从 Store 中查询 thread 归属，拒绝跨用户访问
  3. 路径前缀：Paths.user_dir(user_id) 统一管理所有用户文件路径，_validate_user_id() 防止路径穿越
  4. Store Namespace：user_threads_namespace(user_id) 返回 ("threads", "user:{user_id}") 元组，确保 Store
  查询按用户分区
  5. Runtime 传播：user_id 通过 config["metadata"]["user_id"] 注入到 agent runtime，middleware
  从中读取并传递给下游（memory、sandbox、thread data）