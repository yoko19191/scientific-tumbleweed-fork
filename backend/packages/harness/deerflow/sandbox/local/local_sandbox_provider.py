...
class LocalSandboxProvider(SandboxProvider):
    ...
    def _build_thread_path_mappings(thread_id: str, user_id: str | None = None) -> list[PathMapping]:
        ...
        The caller passes ``user_id`` explicitly from the runtime context so this
        provider matches :class:`AioSandboxProvider` without importing application
        runtime state into the sandbox layer.
        """
        ...
        if user_id:
            user_data_dir = paths.user_thread_dir(user_id, thread_id)
            workspace_dir = paths.user_thread_workspace_dir(user_id, thread_id)
            uploads_dir = paths.user_thread_uploads_dir(user_id, thread_id)
            outputs_dir = paths.user_thread_outputs_dir(user_id, thread_id)
            acp_workspace_dir = paths.user_thread_acp_workspace_dir(user_id, thread_id)
        else:
            user_data_dir = paths.sandbox_user_data_dir(thread_id)
            workspace_dir = paths.sandbox_work_dir(thread_id)
            uploads_dir = paths.sandbox_uploads_dir(thread_id)
            outputs_dir = paths.sandbox_outputs_dir(thread_id)
            acp_workspace_dir = paths.acp_workspace_dir(thread_id)
        ...
        return [
            ...
            PathMapping(
                ...
                local_path=str(acp_workspace_dir),
                read_only=False,
            ),
        ]
    ...
    def acquire(self, thread_id: str | None = None, user_id: str | None = None) -> str:
        ...
        - ``thread_id="abc"`` yields a per-thread ``LocalSandbox`` with id
          ``"local:abc"``. When ``user_id`` is present, the cache key and id are
          user-scoped (``"local:{user_id}:{thread_id}"``) so two users sharing a
          thread id do not share local path mappings.
        ...
        cache_key = f"{user_id}:{thread_id}" if user_id else thread_id
        ...
        new_mappings = list(self._path_mappings) + self._build_thread_path_mappings(thread_id, user_id)
