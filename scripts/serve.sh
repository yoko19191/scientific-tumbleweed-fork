...
# 2. Gateway API
run_service "Gateway" \
    "cd backend && DEER_FLOW_NGINX_CONFIG_PATH='$REPO_ROOT/docker/nginx/nginx.local.conf' PYTHONPATH=. uv run uvicorn app.gateway.app:app --host 0.0.0.0 --port 8001 $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1" \
    8001 30
