...
> 发现总数: ~90 个问题，其中 20 个直接影响用户体验的 bug 已修复 + 1 个密钥泄露风险已修复
...
### S01. `.env` 包含真实 API key（已修复）
- **文件**: `.env`
- **现象**: 文件中包含真实的 Tavily、Jina、Firecrawl、OpenAI、Anthropic、Gemini、DeepSeek、Moonshot、LangSmith API key。虽然 `.env` 已在 `.gitignore` 中且未被 git 追踪，但本地文件仍包含明文密钥。
- **修复**: 所有真实 key 已替换为 `your-xxx-api-key` 占位符。
- **后续建议**: 立即在各平台轮换（rotate）所有已暴露的 key。

### S03. Markdown 渲染 XSS 漏洞（已修复）
- **文件**: `frontend/src/components/workspace/messages/markdown-content.tsx:38`
- **现象**: 链接 href 未过滤 `javascript:`、`data:`、`vbscript:` 等危险协议，攻击者可通过 AI 生成的 markdown 注入恶意链接。
- **修复**: 添加协议白名单（`http://`、`https://`、`mailto:`、`#`、`/`），非白名单协议的链接渲染为 `<span>` 而非 `<a>`。
