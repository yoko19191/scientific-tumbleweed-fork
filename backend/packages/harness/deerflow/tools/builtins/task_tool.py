...
def _summarize_usage(records: list[dict] | None) -> dict[str, int] | None:
    if not records:
        return None
    return {
        ...
        "total_tokens": sum(record.get("total_tokens", 0) or 0 for record in records),
    }
...
async def task_tool(
    ...
    if runtime_app_config is not None:
        available_tools_kwargs["app_config"] = runtime_app_config
    ...
    try:
        while True:
            ...
            if poll_count > max_poll_count:
                ...
                async def cleanup_timed_out_task_when_done() -> None:
                    cleanup_poll_count = 0

                    while True:
                        latest_result = get_background_task_result(task_id)
                        if latest_result is None:
                            return

                        if latest_result.status in {SubagentStatus.COMPLETED, SubagentStatus.FAILED, SubagentStatus.CANCELLED, SubagentStatus.TIMED_OUT} or getattr(latest_result, "completed_at", None) is not None:
                            cleanup_background_task(task_id)
                            return

                        if cleanup_poll_count > max_poll_count:
                            logger.warning(f"[trace={trace_id}] Deferred cleanup for polling-timed-out task {task_id} timed out after {cleanup_poll_count} polls")
                            return

                        await asyncio.sleep(5)
                        cleanup_poll_count += 1

                def log_timed_out_cleanup_failure(cleanup_task: asyncio.Task[None]) -> None:
                    if cleanup_task.cancelled():
                        return

                    exc = cleanup_task.exception()
                    if exc is not None:
                        logger.error(f"[trace={trace_id}] Deferred cleanup failed for polling-timed-out task {task_id}: {exc}")

                asyncio.create_task(cleanup_timed_out_task_when_done()).add_done_callback(log_timed_out_cleanup_failure)
