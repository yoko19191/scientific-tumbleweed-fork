...
from deerflow.agents.lead_agent.config import CHAT_TOOL_GROUPS, COMPUTER_PROFILE, LeadProfile, get_lead_profile
...
def _resolve_effective_tool_groups(agent_config, profile: LeadProfile) -> list[str] | None:
    groups = agent_config.tool_groups if agent_config and agent_config.tool_groups is not None else profile.tool_groups
    if profile.variant != "chat":
        return groups
...
def build_lead_agent(profile: LeadProfile, config: RunnableConfig):
    ...
    effective_profile = COMPUTER_PROFILE if is_bootstrap else profile
    if not is_bootstrap and agent_config is not None and getattr(agent_config, "variant", None):
        effective_profile = get_lead_profile(agent_config.variant)
    is_plan_mode = cfg.get("is_plan_mode", effective_profile.is_plan_mode)
    subagent_enabled = cfg.get("subagent_enabled", effective_profile.subagent_enabled)
    max_concurrent_subagents = cfg.get("max_concurrent_subagents", effective_profile.max_concurrent_subagents)
    sandbox_provider_variant = effective_profile.sandbox_provider_variant
    tool_groups = _resolve_effective_tool_groups(agent_config, effective_profile)
    _write_effective_runtime_context(
        config,
        variant=effective_profile.variant,
        is_plan_mode=is_plan_mode,
        subagent_enabled=subagent_enabled,
        max_concurrent_subagents=max_concurrent_subagents,
        sandbox_provider_variant=sandbox_provider_variant,
    ...
    config["metadata"].update(
        {
            ...
            "is_plan_mode": is_plan_mode,
            "subagent_enabled": subagent_enabled,
            "agent_variant": effective_profile.variant,
            "sandbox_provider_variant": sandbox_provider_variant,
    ...
    if is_bootstrap:
        ...
        return create_agent(
            ...
            system_prompt=apply_prompt_template(
                subagent_enabled=subagent_enabled,
                max_concurrent_subagents=max_concurrent_subagents,
                agent_key=effective_profile.agent_key,
                user_id=user_id,
