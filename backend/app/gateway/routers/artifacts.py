...
@router.get(
    ...
    summary="List Sandbox Root",
    description="List the contents of the sandbox root directory (/mnt/user-data).",
)
async def list_sandbox_root(thread_id: str, request: Request) -> dict:
    return await list_directory(thread_id, "mnt/user-data", request)


@router.get(
    "/threads/{thread_id}/files/{path:path}",
...
async def list_directory(thread_id: str, path: str, request: Request) -> dict:
    ...
    entries = []
    for child in sorted(actual_path.iterdir(), key=lambda p: (not p.is_dir(), p.name.lower())):
